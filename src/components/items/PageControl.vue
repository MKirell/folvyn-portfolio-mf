<template>
  <div v-if="pageCount > 1" class="flex items-center gap-[10px]">
    <button
      ...
      :disabled="page === 0"
      :aria-label="fill(a11y.previousIn, { name: label })"
      @click="emit('previous')"
    >
      <ChevronLeft :size="16" />
    </button>
    <span class="flex items-center gap-[6px]">
      <button
        ...
        :aria-label="fill(a11y.goToPageIn, { name: label, page: p })"
    ...
    <button
      ...
      :aria-label="fill(a11y.nextIn, { name: label })"
      @click="emit('next')"
    >
